...
def create_lang(skel, lang, ref=None):
    ...
    lang_skel = {lang: skel[DEFAULT_LANG]}
    lang_data = merge(lang_skel, read_json(current_dir / f'translations/{PREFIX}.{lang}.json'))

    if ref : 
        # If ref language is defined, merge with ref to get not translated string in ref language
        lang_data[lang] = merge_with_ref(lang_data[lang], ref[DEFAULT_LANG])
...
def update_ts_config(topdir, tsconfig):
    ...
    tr_regex = re.compile(r"i18n\.t\(['\"]%s\.([A-Za-z0-9\._]+)['\"]" % PREFIX)
    msg_ids = set()
    for file in files:
...
for lang in config.get('languages', []):
    ...
    # Write output
    try:
        with open(current_dir / f'translations/{PREFIX}.{lang}.json', 'w') as lang_file:
            json.dump(lang_data, lang_file, indent=2, ensure_ascii=False)
        print(f'Wrote translations/{PREFIX}.{lang}.json')
    except Exception as e:
        print(f'Failed to write translations/{PREFIX}.{lang}.json: {e}')
